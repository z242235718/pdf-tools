...
export default function PdfToPngPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const created = await createTask('pdf_to_png', [file_id], { page_range: 'all', dpi })
      setTask(created)
  ...
  return (
    <section className="page">
      <h1>PDF 转 PNG</h1>
      <p className="text-muted">上传 PDF，按页渲染为 PNG 图片。多页时输出 ZIP 压缩包。</p>
      ...
      {task && (
        <div className="result-card">
          ...
          {task.error_message && <p className="error-msg">{task.error_message}</p>}
        </div>
      )}
    </section>
  )
